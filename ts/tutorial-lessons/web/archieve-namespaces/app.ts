...
// reference to JS existing features in namespaces won't work, until you setup in tsconfig: "outFile": "./bundle.ts". But it won't work with "target": "es6"
// better to use import from

namespace App { // we put all the classes in namespace, so we could use other f(), declared in the namespace in separate referenced     file
